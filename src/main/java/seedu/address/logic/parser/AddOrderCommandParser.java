...
public class AddOrderCommandParser implements Parser<AddOrderCommand> {
    public AddOrderCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_ORDERNAME, PREFIX_QUANTITY, PREFIX_NAME, PREFIX_ADDRESS, PREFIX_DELIVERYDATETIME, PREFIX_PHONE, PREFIX_TAG);

        if (!arePrefixesPresent(argMultimap, PREFIX_ORDERNAME, PREFIX_QUANTITY, PREFIX_NAME, PREFIX_ADDRESS, PREFIX_DELIVERYDATETIME)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddOrderCommand.MESSAGE_USAGE));
        }

        OrderName ordername = OrderParserUtil.parseOrderName(argMultimap.getValue(PREFIX_ORDERNAME).get());
